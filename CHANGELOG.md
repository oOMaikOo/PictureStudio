...
- **Eigene Hilfesektion „Live-Klassifikation"** (`gui/help_dialog.py`, Sektion 21) — die Seite teilte sich bisher behelfsweise die Klassifikations-Hilfe, obwohl sie eigene Bedienelemente hat. Beschreibt Modell- und Quellenwahl (inkl. des Vorrangs des URL-Feldes vor dem Kamera-Dropdown), die drei Einstellungen (Klassifizieren alle N Frames, Mindest-Konfidenz, ROI) und das Lesen des Overlays. Hält zwei Punkte fest, die man der Oberfläche nicht ansieht: beim Übernehmen wird das **vollständige** Bild gespeichert, nicht der ROI-Ausschnitt, und das Bild wird **nicht** automatisch gelabelt. Erreichbar über *Hilfe → Live-Klassifikation – Hilfe* (neuer Schlüssel `menu.help.liveclassify` in DE und EN). Damit hat jede der 17 Seiten eine eigene, passende Hilfesektion, und `SECTIONS` ist lückenlos.
...
- **`gui/widgets/camera_settings_group.py` — gemeinsames `CameraSettingsGroup`-Widget.** `CameraPage` und `CameraCaptureDialog` haben die fünf Eigenschafts-Slider (Helligkeit, Kontrast, Sättigung, Schärfe, Belichtung), den Zurücksetzen-Knopf und das Filter-Dropdown bisher getrennt aufgebaut — Wertebereiche und Defaults standen wörtlich doppelt im Code. Jetzt einmal in `SLIDERS` / `FILTERS`. Das Widget meldet nur, was der Nutzer geändert hat (`prop_changed`, `props_reset`, `filter_changed`); das Anwenden bleibt beim Besitzer, weil genau das sich unterscheidet (`_camera_thread` vs. `_frame_thread`). Besitzerspezifische Zusätze kommen über `add_filter_row()` hinein — so hängt `CameraPage` seine „Auch für Scoring anwenden"-Checkbox ein. Layout und Verhalten unverändert; netto 64 Zeilen weniger in den beiden Konsumenten.
...
- **Datensatz-Exporte blockieren die Oberfläche nicht mehr (BACKLOG T)** — `_export_coco()` und `_export_yolo()` in `gui/pages/data_page.py` liefen synchron, und beide öffnen in `core/dataset.py` jedes Bild einzeln mit PIL, um die Bildgröße zu lesen; bei einem großen Datensatz fror das Fenster ein. Neu: `ExportThread(QThread)` neben dem bestehenden `AnalysisThread`. Der CSV-Export läuft über denselben Weg, obwohl er keine Bilder öffnet — sonst blockiert einer von dreien ohne erkennbaren Grund. Während eines Exports sind die Export-Schaltflächen gesperrt und der Fortschrittsbalken sichtbar.

- **Stille Exception-Handler aufgelöst (BACKLOG S, inkl. F)** — 34 Stellen, an denen ein echter Fehler spurlos verschwand, melden sich jetzt über `log.debug`/`log.warning`/`log.error` mit Kontext. Schwerpunkte: `monitor/runner.py` (Scoring-Ausfall pro Frame, MQTT-Alarm, Notifier-Aufbau — im Daemon lief das bisher blind weiter), `core/dataset.py` (COCO/YOLO-Export überging unlesbare Bildgrößen), `core/data_drift.py` (Bilder fielen unbemerkt aus der Auswertung), `core/model_manager.py` (nicht gelöschte Modelldatei), `core/docker_generator.py` (nicht ausführbares `run_monitor.sh`), `gui/camera_capture_dialog.py` (Event-Log, Sidecars, Grad-CAM). `utils/i18n.py` meldet jetzt, wenn Platzhalter einer Übersetzung nicht zu den Aufruf-Argumenten passen — bisher wurde stumm der Rohtext angezeigt. Acht Module bekamen dafür erstmals einen Logger.

- **`gui/camera_capture_dialog.py` vollständig internationalisiert** — 208 hartkodierte deutsche Strings (157 einfache Literale, 11 mehrzeilige Tooltips, 24 f-Strings mit Interpolation, plus die mehrteilige Profil-Meldung) laufen jetzt über `tr()`. 180 neue `capdlg.*`-Schlüssel in DE und EN; wo möglich wurden bestehende wiederverwendet (`common.error`, `common.no_model`, `common.no_images`, `common.saved`, `common.exported`, `camera.search_running`, `camera.no_camera`, `camera.connect_btn`, `camera.log_btn`, `camera.score_none`, `camera.ip_dialog_prompt`, `camera.refresh_tooltip`). Log-Meldungen (`log.info` / `log.warning`) bleiben bewusst englisch.
- **`gui/qa_review_dialog.py` und `gui/dialogs/model_comparison_dialog.py` erstmals übersetzt** — beide hatten bisher keinen einzigen `tr()`-Aufruf. Die inline gebaute Pluralform im QA-Header (`'s' if n == 1`) ist durch zwei eigene Schlüssel ersetzt (`qa.header_one` / `qa.header_many`), weil sich eine solche Konstruktion nicht übersetzen lässt.
- Locale-Dateien von 891 auf **1105 Schlüssel** gewachsen, DE und EN weiterhin exakt deckungsgleich.

### Fixed

- **Video-Annotation meldete fehlgeschlagenes Speichern als Erfolg** — `_save_to_project()` in `gui/pages/video_annotation_page.py` fing `project.save()` mit `except Exception: pass` ab und zeigte danach unbedingt „N Frames übernommen". Schlug das Speichern fehl, war die Übernahme nicht dauerhaft, der Nutzer erfuhr davon aber nichts. Jetzt: Fehler wird protokolliert und als `QMessageBox.critical` gemeldet, die Erfolgsmeldung entfällt. Frames, die beim Kopieren scheitern, werden gezählt und in der Erfolgsmeldung ausgewiesen statt stillschweigend zu fehlen.
- **Falscher Fallback-Schwellwert bei defektem ONNX-Sidecar** — `core/onnx_anomaly_scorer.py` verschluckte Lesefehler der Sidecar-Datei und arbeitete still mit dem Standard-Schwellwert weiter; das wird jetzt als Warnung protokolliert.

- **Zurücksetzen der Kamera-Einstellungen ließ die Zahlenwerte stehen** — `CameraPage._reset_cam_settings()` schob die Slider mit `blockSignals(True)` zurück, wodurch `valueChanged` ausblieb und die Zahl neben jedem Slider den alten Wert behielt. `CameraSettingsGroup.reset()` aktualisiert die Labels jetzt explizit (Regressionstest: `test_settings_group_reset_updates_value_labels`).
