...
| ID | Beschreibung | Datei(en) |
|----|-------------|-----------|
| O | **Stack 11 + 14 sind unerreichbar** — Die Sidebar-Listen decken `{0–10, 12, 13, 15, 16}` ab; für 11 und 14 existiert kein Nav-Eintrag und kein `_switch_page()`-Aufruf. `DatasetStatsPage` (395 Z.) und `DataDriftPage` (510 Z.) werden trotzdem konstruiert und per `set_project()` befüllt. Dazu tote Tour-Inhalte (`guide_tour.py:765` und `:859`, ~85 Z.). Entscheidung: anbinden oder entfernen | `gui/sidebar.py`, `gui/main_window.py`, `gui/guide_tour.py`, `gui/pages/dataset_stats_page.py`, `gui/pages/data_drift_page.py` |
| P | **Stack-Index als `IntEnum` statt Magic Number** — derselbe Index wird an vier Stellen von Hand gepflegt: Listenposition in `main_window._build_ui()`, Tupel in `sidebar.py`, Dict-Keys in `guide_tour.py`, `PAGE_TO_SECTION` in `help_dialog.py`. Folgeschäden bereits sichtbar: Key `9` fehlt in `PAGE_TO_SECTION` (F1 auf Batch-Inferenz öffnet „Erste Schritte"; eine Batch-Hilfesektion gibt es gar nicht), `SECTIONS` hat Löcher bei 15 und 21 (Reste der entfernten Objekterkennung/Clustering), Stack 16 mappt behelfsweise auf Sektion 7 statt auf eine eigene Live-Klassifikations-Sektion. Ein `IntEnum Page` in einem gemeinsamen Modul entschärft künftiges Renumbering | `gui/main_window.py`, `gui/sidebar.py`, `gui/guide_tour.py`, `gui/help_dialog.py` |
...
| I | **dataset_stats_page: PIL auf QThread** — Die Seite hat **keinen einzigen Thread**. Die Größenanalyse (`:196`) ist auf 200 Samples gedeckelt, die Duplikatsuche (`:228`) läuft aber mit `imagehash` ungedeckelt über *alle* Projektbilder im Main-Thread → UI friert minutenlang ein. `DatasetStatsWorker(QThread)` analog zu `AnalysisThread` in `data_page.py` einführen. Hängt an **O** (Seite ist derzeit gar nicht erreichbar) | `gui/pages/dataset_stats_page.py` |
