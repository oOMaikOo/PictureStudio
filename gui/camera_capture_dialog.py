...
class CameraCaptureDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        self._cam_settings = CameraSettingsGroup(
            cam_props=self._initial_cam_props, filter_name=self._active_filter)
        self._cam_settings.prop_changed.connect(self._apply_cam_prop)
        self._cam_settings.props_reset.connect(self._apply_cam_props)
        self._cam_settings.filter_changed.connect(self._on_filter_changed)
    ...
    def _connect(self) -> None:
        ...
        if tab == 2:  # Video-Datei
            ...
            current_props = self._cam_settings.cam_props()
    ...
    def _apply_cam_props(self, props: dict) -> None:
        """Forward a whole set of camera properties to the live frame thread."""
        if self._frame_thread and self._frame_thread.isRunning():
            self._frame_thread.set_cam_props(props)

    def _on_filter_changed(self, filter_name: str) -> None:
        """Store the preprocessing filter selected in the settings widget."""
